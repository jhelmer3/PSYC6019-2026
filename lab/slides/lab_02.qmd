---
title: "Data Workflow and Visualization with the `tidyverse`"
...
head(read.csv(here::here("lab", "data", "world_cup_player_stats.csv")))
```
...
::: {.column width=33%}
Still need to separate by position

Can use `facet_wrap(~ grouping_variable)` to make different panels

Can add `position` to the `fill` mapping aesthetic, while we're at it, to add a bit of color.
:::
...
We can layer on the observed data by adding a point plot with `geom_point()`.
...
To plot two continuous variables, we can specify both the x- and y-axes in `aes()` and add a point plot with `geom_point()`. We can also start to layer `geom`s, adding a trend line with `geom_smooth()`. Note that `geom_smooth()` picks up the same x and y assignment as already specified in `aes()`.
