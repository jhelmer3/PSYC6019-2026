...
We can visualize our data with the `ggplot2` package. `ggplot2` provides a very expansive and flexible plotting system. It is also at the core of a whole ecosystem of `ggplot2` extension packages, meaning so many plotting options are available.
...
#| fig-height: 4.5
ggplot(data = player_stats,
       mapping = aes(x = goals, fill = position, y = after_stat(prop))) +
  geom_bar() +
  facet_wrap(~ position)
```
...
::: {.column width=33%}
Let's add some nicer-looking labels to the facets.

We need to create a vector of the pattern `c(value_in_the_data = label_we_want, ...)`.

Then, we can tell `facet_wrap()` how to interpret it with the `as_labeller()` function.
:::
...
Violin plots help us easily compare distributions. Create them with `geom_violin()`. Specify the groups on the y-axis by specifying `aes(y = position)`.

We can group the violins by specifying the `y` value in the `mapping`. [If we want to layer on the observed data by adding a point plot, do we need to change the `mapping` or the `geom`?]{.fragment} [`geom_jitter()`!]{.fragment}
...
#| fig-height: 4.5
ggplot(data = player_stats,
       mapping = aes(x = minutes_played, y = position)) +
  geom_violin()
```
...
#| fig-height: 4.5
ggplot(data = player_stats,
       mapping = aes(y = minutes_played, x = position)) +
  geom_violin() +
  geom_jitter()
```
...
An note about `geom_jitter()`: by default it will add noise in both the x- and y-direction (`width` and `height` arguments). This may not be appropriate for all cases: here, the default behavior hides that values in category 2 are rounded.
...
::: {.column width=50%}
If we wanted to see the points with color representing player position, would we need to change the `mapping` or `geom`? [`aes(color = position)`!]{.fragment}
:::
...
#| fig-height: 4.5
ggplot(data = player_stats,
       mapping = aes(x = minutes_played, y = goals, color = position)) +
  geom_point() +
  geom_smooth()
```
...
#| fig-height: 4.5
player_stats |>
  filter_out(position == "GK") |>
  ggplot(mapping = aes(x = minutes_played, y = goals, color = position)) +
  geom_point() +
  geom_smooth()
```
...
player_stats |>
  filter_out(position == "GK") |>
  ggplot(mapping = aes(x = minutes_played, y = goals, 
...
::: {.column width="33%"}
[ggplot2: Elegant Graphics for Data Analysis (3e)](https://ggplot2-book.org/){target="blank"}

[![](../media/lab_01/ggplot2.jpg)](https://ggplot2-book.org//){target="blank"}
:::
...
Back to the violin plot, we can layer on the observed data by adding a point plot with `geom_point()`. 

:::: {.columns style="font-size:60%"}

::: {.column width=33%}
But, the points are all overlapping and hard to see at the different levels of position.

We can nudge them off their data value by using `geom_jitter()`. But, we only want it to nudge on the y-axis, not the x-axis.

So, we will need to set `geom_jitter()`'s `width` argument to 0. Otherwise, it'll try to move the points on their `minutes_played` value, too.
:::
