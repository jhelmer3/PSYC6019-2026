---
title: "Chi-Squared Tests and T-Tests"
...
```{r}
#| echo: false
tibble(type = c("Undergrad", "Graduate"),
       in_state = c(25, 5),
       out_of_state = c(10, 10)) |>
  gt(rowname_col = "type") |>
  cols_label(in_state = "In-State",
...
```{r}
#| echo: false
tibble(type = c("Undergrad", "Graduate", "Total"),
       in_state = c(25, 5, NA),
       out_of_state = c(10, 10, NA),
       Total = c(NA, NA, NA)) |>
  gt(rowname_col = "type") |>
  cols_label(in_state = "In-State",
             out_of_state = "Out-of-State") |>
...
```{r}
#| code-line-numbers: false
studdat <- tibble(type = c("Undergrad", "Graduate"),
                  in_state = c(25, 5),
                  out_of_state = c(10, 10))
studdat
...
::: {.column width="40%"}
[○ `correct` = [`T`/`F`], applies a "continuity correction" to the $\chi^2$ value. Default is `T`.]{.subpoint}
:::
::::
