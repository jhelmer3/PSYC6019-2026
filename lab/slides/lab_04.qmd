...
tibble(parent_in_stem = c("No", "Yes", "Total"),
       `Non-STEM School` = c(91*115/200, 91*85/200, 59+56) |> round(),
       `STEM School` = c(109*115/200, 109*85/200, 109) |> round(),
...
table(major_data$parent_in_stem, major_data$school_type) |>
  as.data.frame() |>
  as_tibble() |>
  set_names(c("parent_in_stem", "school_type", "n"))
```
:::
::::
...
chisq.test(chisqdat)
```
[We can reject the null in favor of the alternative that students' school type (STEM vs. non-STEM) and whether they choose a STEM or non-STEM major are not independent.]{.fragment}
:::
::::
