...
Let's say a researcher claims the average city miles per gallon across all cars is 30mpg. They collect a sample of 234 cars and would like you to test this. We do not know the population standard deviation.
...
`geom_qq()` adds the points, `geom_qq_line()` adds the expected line.
...
```{r}
t.test(ice_cream_cleaned$vanilla, ice_cream_cleaned$cookie_dough, paired = T)$stderr
```
...
Not many data points, but seems OK!
