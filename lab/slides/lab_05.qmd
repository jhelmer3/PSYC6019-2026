...
Let's you work for a car manufacturer, and a researcher claims the average city miles per gallon across all cars is 30mpg. They collect data on a sample of 234 cars (`mpg` dataset) and would like you to test this. We do not know the population standard deviation.
...
::: column
```{r}
cty_ttest <- t.test(mpg$cty, mu = 30, 
...
`t.test(outcome ~ group_var, data, var.equal = F)`
...
## Paired Samples *t*-Test Plot

Can plot the data and means to compare! Which would we give the promotion to?
...
#| fig-align: center
ice_cream_sales |>
  ggplot(aes(x = flavor, y = sales, fill = flavor, color = flavor)) +
  ...
  guides(x = guide_axis(cap = T),
         ...
         fill = "none",
         color = "none") +
  scale_x_discrete(NULL, labels = c("cookie_dough" = "Cookie Dough",
                                    "vanilla" = "Vanilla")) +
  scale_y_continuous("Sales", labels = scales::label_currency()) +
  scale_fill_manual(values = c("cookie_dough" = "#D6A15D",
                               "vanilla" = "#8FB8A8")) +
  scale_color_manual(values = c("cookie_dough" = "#B47735",
                                "vanilla" = "#5E8F7D")) +
  theme_classic() +
...
```{r}
#| eval: false
# partially GPT-generated code (I didn't want to pick colors)
ice_cream_sales |>
  ggplot(aes(x = flavor, y = sales, fill = flavor, color = flavor)) +
  geom_violin(alpha = 0.3) +
  geom_jitter(alpha = 0.6, shape = 16, size = 2) +
