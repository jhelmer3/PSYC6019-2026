...
dat <- dat |> group_by(Subject) |>
  mutate(sec_subj = 1:n() - 1,
         Subject = Subject + sec_subj)
dat$sec_subj <- NULL
```
...
The reduced model made better predictions (lower error), which might be evidence against treating the groups as having meaningfully different means
...
- We don't have super clear evidence one way or the other about whether the full or reduced model makes better predictions
- Partly a function of the cost associated with splitting the sample in half
