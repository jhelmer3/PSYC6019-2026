...
- If we collected repeated samples of size 100, what is $\sigma_{\bar{x}}$? 
    - [$\sigma_{\bar{x}} = \frac{10}{\sqrt{100}} = 1$ and $\bar{x} \sim N(\mu_{\bar{x}} = 1, \sigma_{\bar{x}}=1)$]{.answer .fragment .content-visible unless-meta="params.v0"}
...
In power analysis, we assume an effect at the **population level**. What do you think we would call an equivalent population effect size (Hint: not an English letter!)
...
The Type I error rate $\alpha$ determines the cutoff between the retention and rejection regions. Increasing $\alpha$ means you are likely to reject $H_0$ for both the null and alternative hypothesis
...
- Making way more Type I error's than our actual $\alpha = .05$
