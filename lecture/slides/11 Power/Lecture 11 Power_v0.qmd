---
title: "Lecture 11: Power and Power Analysis"
...
- [Increase $\alpha$]{.answer .fragment .content-visible unless-meta="params.v0"}
...
## Increasing $\alpha$ Decreases Power
...
plot2 <- ggplot(x, aes(x), color = c(my_green, my_red)[ind]) + 
  ...
  theme_bw() + 
  labs(x = expression(bar(x)), y = "Density", 
       title = expression(paste(alpha, "= .10, N = 16, d = .5, Power = 0.64")))  +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        axis.ticks.y = element_blank(),
        axis.text.y = element_blank())  
