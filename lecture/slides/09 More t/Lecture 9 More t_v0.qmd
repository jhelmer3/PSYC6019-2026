...
optim <- read.csv("../../data/Armor_Replication_Lecture8.csv")
...
-   The observations are independent
    -   [Make sure no datapoints come from the same person or clusters that are somehow related]{.answer .fragment .content-visible unless-meta="params.v0"}
...
```{r}
p1 <- ggplot(optim) + geom_histogram(aes(Best_mean, y = ..density..), bins = 9, na.rm = TRUE) +
  stat_function(fun = dnorm, args = list(mean = 1.28, sd = 1.09), col = my_red, size = I(1)) + labs(x = "Optimism Ratings") + 
  theme_light() + theme(axis.title = element_text(size = 14))

p2 <- ggplot(optim, aes(sample = Best_mean)) + geom_qq(na.rm = TRUE) + geom_qq_line(na.rm = TRUE, col = my_red, size = I(1)) +
  theme_light() + theme(axis.title = element_text(size = 14))
...
skewness(optim$Best_mean)
kurtosis(optim$Best_mean)
...
#| echo: true
#| warning: false
shapiro.test(bball$MP) ## basketball data
ks.test(bball$MP, "pnorm") ## basketball data
shapiro.test(optim$Best_mean) ## optimism data
ks.test(optim$Best_mean, "pnorm") ## optimism data
```
...
2.  Transform these values back onto the scale of the original variable using the sample mean and standard error ($s_{\bar{x}}$). How do we do this?
...
```{r}
#| echo: true
qt(p = .05/2, df = 43, lower.tail = FALSE)

```

Why did I set $p = \frac{.05}{2}$?

[For a two-tailed test, we want 2.5% of the distribution in both the lowr and upper tails]{.answer .fragment .content-visible unless-meta="params.v0"}
...
```{r}
#| echo: true
t_crit <- qt(p = .05/2, df = 31, lower.tail = FALSE)
```
...
pt(t_xbar1_xbar2, df = n1 + n2 - 2, lower.tail = TRUE) * 2
```
...
```{r}
#| echo: true
(t_crit <- qt(.05/2, df = n1 + n2 - 2, lower.tail = FALSE))
```
...
The actual adjustment happens on the **degrees of freedom**. The Welch-Satterthwaite correction adjusts the degrees of freedom so they become smaller than $(n1 + n2 - 2)$. 
