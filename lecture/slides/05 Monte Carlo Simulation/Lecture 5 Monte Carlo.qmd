...
- **Example:** I can generate numbers from a Bernoulli distribution with $\pi = .5$ to represent my coin flips.

- Random number generating functions in `R` typically start with `r` and then the name of the probability distribution. 

- In this case, I'm actually going to use the `rbinom()` function. Why not `rbernoulli()`?
...
-  Takes our original function: $X(\text{coin flip}) = \text{heads} = 1; \text{tails} = 0$
- Inverts it: $X^{-1}(X) = 1 = \text{heads}; 0 = \text{tails}$


## Pseudo Randomness

-   Computers are not capable of generating truly random sequences of numbers
-   However, the sequence before observing repetition can be so large that it becomes functionally random
...
set.seed(521)
outcome <- rbinom(n = flips, size = 1, prob = p)
outcome
mean(outcome) ## proportion heads
```
...
- I'll start by creating an empty vector, then initialize my `for` loop
<<<<<<< Updated upstream
- The for loop uses an index variable, which I am naming `i`, to index iterations. 
- I will save my result into the `i`th entry of my empty vector
=======
- The for loop uses an index variable, which I am naming `i`, to index iterations. - I will save my result into the `i`th entry of my empty vector
>>>>>>> Stashed changes
...
original_result <- numeric(iter) ## empty vector to save results
altered_result <- numeric(iter) ## empty vector to save results
set.seed(124)
for(i in 1:iter){
  outcome <- rbinom(n = 10, size = 1, prob = p)
  original_result[i] <- mean(outcome) ## save raw result
  outcome[5] <- 0
  altered_result[i] <- mean(outcome) # save altered result
}
...
```{r}
df <- data.frame(
  value = factor(c(original_result, altered_result), levels = seq(0, 1, by = 0.1)),
  group = rep(c("Original", "Altered"), each = 1000)
)

ggplot(df, aes(x = value, fill = group)) +
  geom_bar(position = position_dodge(preserve = "single"), alpha = 0.85) +
  scale_fill_manual(values = c("Original" = "#377eb8", "Altered" = "#e41a1c")) +
...
We will make use of Monte Carlo throughout the semester, often using the random *normal* distribution functions: `rnorm(n, mean, sd)`

There are also `pnorm` (probability), `qnorm` (quantile), and `dnorm` (density) functions that are all part of the same families that we may make use of
...
<iframe src="rnorm_help.html" width="100%" height="500" style="border:none;">
