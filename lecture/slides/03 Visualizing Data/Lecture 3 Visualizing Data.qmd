...
## Example: The Data/Ink Ratio

![](../../Figs/dataink.png)

## Can You Identify Chartjunk?
...
The following slides will show some examples of pairs of graphs of the same data. Which do you think is the better representation of the data?
...
This is the default of players by position in `ggplot2` if I don't do any manual tweaks. How could I make this better?
...
-   I ordered the x-axis in a sensible way. I could have also done something like this (arguably better!)
:::
...
[The mean is a measure of *centrality*. Bars imply there is a difference between values below the bar (colored) and above the bar (not colored)]{.answer .fragment .content-visible unless-meta="params.v0"}
...
Kerns, S. H., & Wilmer, J. B. (2021). Two graphs walk into a bar: Readout-based measurement reveals the Bar-Tip Limit error, a common, categorical misinterpretation of mean bar graphs. *Journal of Vision*, *21*(12), Article 17. <https://doi.org/10.1167/jov.21.12.17>
:::
...
-   Assume that **every** value represents an underlying distribution (kernel function) (e.g., normal)
...
## Violins and Boxes with Means

```{r}
ggplot(bball, aes(y = PTS, x = Position, fill = Position)) + 
  geom_violin(alpha = .1) +
  geom_boxplot(width = .25, alpha = .15) +
  stat_summary(fun = "mean", geom = "point", shape = 20, size = 5, aes(color = Position)) + 
  coord_flip() + 
  theme_light() +
  scale_fill_viridis_d() +
...
-   Preserve **distributions**
    -   don't just plot means/summary statistics
-   Get creative
